...
import { TrainingRawData, TrainingsRepository, TrainingType } from '../application/trainings.repository';
...
export class KyselyTrainingsRepository extends BaseRepository<DB> implements TrainingsRepository {
  ...
  async findOneById({ id }: { id: number }, trx?: Transaction<DB>): Promise<TrainingEntity | null> {
    const result = await this.db(trx).selectFrom('trainings').where('id', '=', id).selectAll().executeTakeFirst();
  ...
  async findActive(trx?: Transaction<DB>): Promise<TrainingEntity | null> {
    ...
    query = query.where((eb) => {
      const conditions = [eb('end_date', 'is', null), eb('start_date', '>=', today), eb('start_date', '<', tomorrow)];
  ...
  async create(data: TrainingRawData['insertable'], trx?: Transaction<DB>): Promise<TrainingEntity | null> {
    const result = await this.db(trx)
  ...
  async delete({ id }: { id: number }, trx?: Transaction<DB>): Promise<boolean> {
    const result = await this.db(trx).deleteFrom('trainings').where('id', '=', id).executeTakeFirst();
