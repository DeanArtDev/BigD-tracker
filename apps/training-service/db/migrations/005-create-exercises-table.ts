...
export async function up(db: Kysely<any>): Promise<void> {
  await db.schema
    ...
    .addCheckConstraint('exercise_types_values', sql`value in ('WORM-UP', 'POST-TRAINING', 'AEROBIC', 'ANAEROBIC')`)
  ...
  await db.schema
    ...
    .addColumn('training_id', 'integer', (col) => col.references('trainings.id').onDelete('cascade'))
    .addColumn('training_template_id', 'integer', (col) => col.references('trainings_templates.id').onDelete('cascade'))
    ...
    .addColumn('type', 'text', (col) => col.references('exercise_types.value').onDelete('restrict').notNull())
