...
describe('AssignTaskToInboxUseCase', () => {
  ...
  test('assigns origin task to inbox', async () => {
    const userId = 77;
    const taskId = 7001;
    const inboxId = 9901;
    const recurrenceId = 8001;
    const task = getTask({ id: taskId, userId, groupId: 9001, recurrenceId });
    ...
    const inboxGroupCheckerService = {
      ensureTaskNotInInboxGroup: jest.fn().mockResolvedValue({ inboxId, success: false }),
    };
    ...
    const useCase = new AssignTaskToInboxUseCase(
    ...
    expect(inboxGroupCheckerService.ensureTaskNotInInboxGroup).toHaveBeenCalledWith({ userId, taskId }, { trx });
    ...
    expect(taskArg.groupId).toBe(inboxId);
    expect(replaceTrxArg).toBe(trx);

    expect(taskOverrideService.updateGroupIdForManyOverrides).toHaveBeenCalledWith(
      { userId, groupId: inboxId, recurrenceId },
      trx,
    );
  });

  test('assigns virtual task series to inbox', async () => {
    const userId = 78;
    const taskId = 7002;
    ...
    const sourceTask = getTask({ id: taskId, userId, groupId: 9002, recurrenceId });
  ...
  test('assigns override task series to inbox', async () => {
    const userId = 79;
    const taskId = 7003;
    ...
    const recurrence = getTaskRecurrence({ id: recurrenceId, userId, taskId });
    const sourceTask = getTask({ id: taskId, userId, groupId: 9003, recurrenceId });
