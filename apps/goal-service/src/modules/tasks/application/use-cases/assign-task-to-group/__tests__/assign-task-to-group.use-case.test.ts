...
describe('AssignTaskToGroupUseCase', () => {
  ...
  test('assigns origin task to group', async () => {
    const userId = 77;
    const taskId = 7001;
    const groupId = 9001;
    const recurrenceId = 8001;
    const task = getTask({ id: taskId, userId, recurrenceId });
    ...
    expect(taskArg.groupId).toBe(groupId);
    expect(replaceTrxArg).toBe(trx);

    expect(taskOverrideService.updateGroupIdForManyOverrides).toHaveBeenCalledWith(
      { userId, groupId, recurrenceId },
      trx,
    );
  });

  test('assigns virtual task series to group', async () => {
    const userId = 78;
    const taskId = 7002;
    const groupId = 9002;
    ...
    const sourceTask = getTask({ id: taskId, userId, recurrenceId });
  ...
  test('assigns override task series to group', async () => {
    const userId = 79;
    const taskId = 7003;
    const groupId = 9003;
